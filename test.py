...
class Tester(object):
    def __init__(self, args):
        ...
        if os.path.isfile(args.ckpt):
            checkpoint = torch.load(args.ckpt,map_location="cuda",weights_only=True)
            model_dict = {}
    ...
    def testing(self):
        ...
        with torch.no_grad():
            ...
            for _, (x, label, fn) in loop:
                ...
                store_restored(pred, label, fn, "/".join(self.args.ckpt.split("/")[:-1]))
...
def main():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="output/WWE-UIE/2025-11-29 14:59:34/LSUI/best_model.pth",
    )
